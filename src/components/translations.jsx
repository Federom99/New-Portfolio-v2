const translations = {
    en: {
      home: 'Home',
      skills: 'Skills',
      projects: 'Projects',
      certificates: 'Certificates',
      cv: "Download Resume",
      greeting: "Hello, I'm Federico Romero!",
    role: "FullStack Software Engineer.",
    introduction: [
      "At 24 years old, I am constantly looking for opportunities to grow and learn in the world of technology.",
      "My approach at work is based on organization and responsibility.",
      "I am passionate about teamwork and adapt quickly to any situation.",
      "I have a C1 level of English and I am determined to perfect it even further."
    ],
    diagonalText: "In this portfolio, you can learn more about me. My technical and soft skills. My projects, my certifications. You can also choose between a light or dark theme. And you can translate to English or Spanish. You can also check my contributions in GitLab or GitHub. And you can contact me on LinkedIn or by Email. I am open to new opportunities and challenges. Do not hesitate to contact me!"
      // Agrega más traducciones según sea necesario
    },
    es: {
      home: 'Inicio',
      skills: 'Habilidades',
      projects: 'Proyectos',
      certificates: 'Certificados',
      cv: "Descargar CV",
      greeting: "¡Hola, soy Federico Romero!",
      role: "Ingeniero de Software FullStack.",
      introduction: [
        "A los 24 años, estoy constantemente buscando oportunidades para crecer y aprender en el mundo de la tecnología.",
        "Mi enfoque en el trabajo se basa en la organización y la responsabilidad.",
        "Me apasiona el trabajo en equipo y me adapto rápidamente a cualquier situación.",
        "Tengo un nivel C1 de inglés y estoy decidido a perfeccionarlo aún más."
      ],
      diagonalText: "En este portafolio, puedes aprender más sobre mí. Mis habilidades técnicas y blandas. Mis proyectos, mis certificaciones. También puedes elegir entre un tema claro u oscuro. Y puedes traducir al inglés o español. También puedes ver mis contribuciones en GitLab o GitHub. Y puedes contactarme en LinkedIn o por correo electrónico. Estoy abierto a nuevas oportunidades y desafíos. ¡No dudes en contactarme!"
      // Agrega más traducciones según sea necesario
    },
  };
  
  export default translations;
  